...
namespace entry
{
	...
	struct Context
	{
		...
		int32_t run(int _argc, const char* const* _argv)
		{
			...
			EMSCRIPTEN_CHECK(emscripten_set_touchstart_callback(canvas, this, true, touchCb) );
			EMSCRIPTEN_CHECK(emscripten_set_touchend_callback(canvas, this, true, touchCb) );
			EMSCRIPTEN_CHECK(emscripten_set_touchmove_callback(canvas, this, true, touchCb) );
			EMSCRIPTEN_CHECK(emscripten_set_touchcancel_callback(canvas, this, true, touchCb) );
	...
	EM_BOOL Context::touchCb(int _eventType, const EmscriptenTouchEvent* _event, void* _userData)
	{
		BX_UNUSED(_userData);

		if (_event)
		{
			s_ctx.m_mx = _event->touches[0].targetX;
			s_ctx.m_my = _event->touches[0].targetY;
			switch (_eventType)
			{
				case EMSCRIPTEN_EVENT_TOUCHSTART:
					s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, 0);
					s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, 0, MouseButton::Left, true);
					bx::printf("EMSCRIPTEN_EVENT_TOUCHSTART\n");
					return true;
				case EMSCRIPTEN_EVENT_TOUCHEND:
					s_ctx.m_eventQueue.postMouseEvent(s_defaultWindow, s_ctx.m_mx, s_ctx.m_my, 0, MouseButton::Left, false);
					bx::printf("EMSCRIPTEN_EVENT_TOUCHEND\n");
					return true;
